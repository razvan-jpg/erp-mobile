...
enum ClientAccountXLSBuilder {
    static func writeTemporaryXLS(from snapshot: ClientAccountSnapshot) throws -> URL {
        let content = makeSpreadsheetML(from: snapshot)
        let safeName = sanitizedFileName(snapshot.client.denumire)
    ...
    private static func makeSpreadsheetML(from snapshot: ClientAccountSnapshot) -> String {
        ...
        rows.append(dataRow([stringCell(L10n.tr("account.client_sheet"))], style: "Title"))
        rows.append(dataRow([stringCell(client.denumire)], style: "Title"))
        rows.append(dataRow([
        ...
        rows.append(dataRow([stringCell(L10n.tr("account.client_data"))], style: "Section"))
        rows.append(dataRow([
            stringCell(L10n.tr("account.partner_role_label")),
            stringCell(snapshot.partnerRole.displayLabel)
        ]))
        rows.append(dataRow([stringCell(L10n.tr("common.cui")), stringCell(value(client.cui))]))
        rows.append(dataRow([stringCell(L10n.tr("common.field_nr_reg_com")), stringCell(value(client.nrRegCom))]))
        rows.append(dataRow([stringCell(L10n.tr("common.field_address")), stringCell(value(client.adresa))]))
        rows.append(dataRow([stringCell(L10n.tr("common.field_iban")), stringCell(value(client.iban))]))
        rows.append(dataRow([stringCell(L10n.tr("common.field_email")), stringCell(value(client.email))]))
        rows.append(dataRow([stringCell(L10n.tr("common.field_phone")), stringCell(value(client.telefon))]))
        rows.append(dataRow([
            stringCell(L10n.tr("clients.field_payment_term_days")),
            stringCell(client.nrZileScadenta > 0 ? String(client.nrZileScadenta) : "0")
        ]))
        rows.append(dataRow([
            stringCell(L10n.tr("common.field_status")),
            stringCell(client.isActive ? L10n.tr("common.active") : L10n.tr("common.inactive"))
        ]))
        rows.append(emptyRow())
