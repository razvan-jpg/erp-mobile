...
struct SupplierAccountSnapshot: Sendable {
    let company: Company?
    let supplier: Supplier
    let ledgerEntries: [SupplierAccountLedgerEntry]
...
enum SupplierAccountPDFBuilder {
    ...
    static func makePDF(from snapshot: SupplierAccountSnapshot) -> Data {
        ...
        return renderer.pdfData { context in
            ...
            y = drawText(L10n.tr("account.supplier_sheet"), at: y, pageWidth: pageRect.width, font: titleFont)
            y = drawText(supplier.denumire, at: y + 4, pageWidth: pageRect.width, font: sectionFont)
            y += 8

            y = drawSection(L10n.tr("account.supplier_data"), at: y, pageWidth: pageRect.width, font: sectionFont)
            y = drawLines([
                line(L10n.tr("account.partner_role_label"), snapshot.partnerRole.displayLabel),
                line(L10n.tr("common.cui"), supplier.cui),
                line(L10n.tr("common.field_nr_reg_com"), supplier.nrRegCom),
                line(L10n.tr("common.field_address"), supplier.adresa),
                line(L10n.tr("common.field_iban"), supplier.iban),
                line(L10n.tr("common.field_email"), supplier.email),
                line(L10n.tr("common.field_phone"), supplier.telefon),
                ...
                line(
                    ...
                    supplier.isActive ? L10n.tr("common.active") : L10n.tr("common.inactive")
                )
            ], at: y, pageWidth: pageRect.width, font: bodyFont)
            y += 8
    ...
    static func writeTemporaryPDF(from snapshot: SupplierAccountSnapshot) throws -> URL {
        let data = makePDF(from: snapshot)
        let safeName = snapshot.supplier.denumire
    ...
    private static func ledgerRowValues(for entry: SupplierAccountLedgerEntry, moneda: String) -> [String] {
        [
            "\(entry.nrCrt)",
    ...
    private static func ledgerRowColors(for entry: SupplierAccountLedgerEntry) -> [UIColor?] {
