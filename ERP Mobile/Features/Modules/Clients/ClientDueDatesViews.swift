...
struct ClientDueDatesListView: View {
    ...
    private var sections: [ClientDueDateCategorySection] {
        ClientDueDatesGrouping.buildSections(from: invoices)
    }

    private var calendarDays: [ClientDueDateCalendarDay] {
        ClientDueDatesCalendarGrouping.buildCalendar(from: invoices)
    }
        ...
        VStack(spacing: 0) {
            ...
            Group {
                if !hasAnyDueDates && !isLoading {
                    ...
                    case .calendar:
                        ScrollView {
                            ClientDueDatesCalendarView(days: calendarDays) { row in
                                openAccount(for: row)
                            }
                            .padding(.vertical, 8)
                        }
        ...
        .appSafeAreaInsetBottom(spacing: 0) {
            VStack(spacing: 0) {
                ...
                if !isLoading, viewMode == .list {
                    VStack(spacing: 0) {
                        Divider()
                        ClientDueDatesCategoryChartView(sections: sections)
                    }
    ...
    private func openAccount(for row: DueDateCalendarClientRow) {
        let summary = ClientDueSummary(
    ...
    private func openAccount(
        ...
        Task {
            ...
            errorMessage = nil
            do {
                let client = try await ClientService.fetchClient(id: row.clientId)
                let listRow = try await ClientService.fetchClientListRows()
                    .first(where: { $0.client.id == row.clientId })
                    ?? ClientListRow(
                        client: client,
                ...
                selectedAccount = ClientAccountContext(
                    row: listRow,
...
private struct DueDateCategoryHeaderView: View {
    let section: ClientDueDateCategorySection
...
private struct ClientDueSummaryRowView: View {
    let row: ClientDueSummary
