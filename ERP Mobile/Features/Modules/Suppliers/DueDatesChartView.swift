...
private struct DueDateChartItem: Identifiable {
...
struct DueDatesCategoryChartView: View {
    let sections: [DueDateCategorySection]

    private var currencyCode: String {
        sections.first(where: { $0.categoryTotal > 0 })?.moneda ?? sections.first?.moneda ?? "RON"
    }

    private var items: [DueDateChartItem] {
